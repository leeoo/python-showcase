...
from MobiFormat import pdb_header_format, ior_format, PalmDOC_header_format, kf8_header_format, exth_header_format, exth_mdata_format, guess_types, MOBI_HEADER_LEN, THUMB_MAX_SIZE, NONE_OFFSET, header_len, he_type, exth_mdata_name
import utils
...
class KindleEBook(ByteBlock):
    ...
    def _parse_header_record(self, record):
        ...
        #
        if mobi_header.sect('kf_version').value() == 6:
    ...
    def _parse_exth_mdata(self, record):
        ...
        mdata = record.split_last_sect(length, name=name, vt=exth_mdata_format[mtype][0])
    ...
    def _determine_record_type(self, head_rec_id):
        ...
        # 封面、封面缩略图
        cover_record_offset = record.sect('201(Cover_Offset)').value()
        thumb_record_offset = record.sect('202(Thumb_Offset)').value()
    ...
    def is_thumbnail_replacible(self):
        ...
        thumb = Image.open(io.StringIO(data))
        #print 'Cover thumbnail size is', thumb.size, 
        if not ( thumb.size[0] < THUMB_MAX_SIZE[0] and thumb.size[1] < THUMB_MAX_SIZE[1] ):
            #print 'and does not need replacing.'
            return False
        #print 'and will be small in KPW, for which maximum size is', THUMB_MAX_SIZE
        #
        data = self.record(self.cover_rec_id).raw_data()
        cover = Image.open(io.StringIO(data))
        #print 'Cover size is', cover.size,
        if cover.size[0] < THUMB_MAX_SIZE[0] and cover.size[1] < THUMB_MAX_SIZE[1]:
            #print 'and is too small for generating thumbnail.'
            return False
        #print 'and will be used to generate new thumbnail.'
        return True

    def replace_thumbnail(self, new_book_filename):
        '''替换 AZW3 文件的封面缩略图为 220*330 像素。
        解决 AZW3 在 KPW 的 cover view 里封面缩略图偏小的问题。'''
        # 利用文件里的封面生成新的封面缩略图
        data = self.record(self.cover_rec_id).raw_data()
        #f = open('cover.jpg', 'wb')
        #f.write(data)
        #f.close()
        cover = Image.open(io.StringIO(data)).convert('RGB')
        cover.thumbnail((220, 330))
    ...
    def extract(self, filename, asin):
        ...
        origin_rec_ids = list(range(self.second_header_rec_id, self.second_header_rec_id + insert_offset))
